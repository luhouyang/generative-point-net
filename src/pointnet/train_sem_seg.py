...
import argparse
import os
from src.pointnet.model import PointNetSemSeg, PointNetSemSegLoss
from src.pointnet.dataset import S3DISDataset
import torch
import datetime
import logging
...
def main(args):
    ...
    experiment_dir = Path('./log_pointnet/')
    ...
    root = 'D:/storage/s3dis/stanford_indoor3d/'
    NUM_CLASSES = 13
    ...
    criterion = PointNetSemSegLoss().cuda()
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            ...
            if mIoU >= best_iou:
                best_iou = mIoU
                logger.info('Save model...')
                savepath = str(checkpoints_dir) + '/best_model_pointnet.pth'
                log_string('Saving at %s' % savepath)
                state = {
                    'epoch': epoch,
                    'class_avg_iou': mIoU,
